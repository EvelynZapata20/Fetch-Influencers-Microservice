...
class Influencer(BaseModel):
    ...
    def is_significant(self) -> bool:
        ...
        if self.followers <= 10000:
            ...
            return (
                self.engagement_visualization_rate >= min_engagement_per_type["micro"]
            )
        elif self.followers <= 1000000:
            return (
                self.engagement_visualization_rate >= min_engagement_per_type["macro"]
            )
        elif self.followers <= 10000000:
            return self.engagement_visualization_rate >= min_engagement_per_type["mega"]
        elif self.followers <= 10000000:
            return (
                self.engagement_visualization_rate
                >= min_engagement_per_type["celebrity"]
            )
        else:
            return True
